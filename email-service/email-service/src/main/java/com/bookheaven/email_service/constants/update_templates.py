def build_template(title, icon, content):
    base = f'''    <!DOCTYPE html>
    <html>
    <head>
      <meta charset="UTF-8">
            ...
            <table width="540" cellspacing="0" cellpadding="0" style="background:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 10px 25px -5px rgba(0,0,0,0.1);">
              ...
              <tr>
                <td align="center" style="background: linear-gradient(135deg, #4f46e5 0%, #7c3aed 100%); padding:35px 20px;">
                  ...
                  <div style="font-size:15px; color:#e0e7ff; margin-top:5px; font-weight:500;">{title}</div>
                </td>
              </tr>
              <!-- Body -->
              <tr>
...
otp_content = '''                  <p style="margin-top:0;">Hello,</p>
...
reset_content = '''                  <p style="margin-top:0;">Hello,</p>
...
order_conf_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
pay_fail_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
welcome_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
seller_order_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{SELLER_USERNAME}}</b>,</p>
...
welcome_seller_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
order_shipped_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
order_delivered_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
missing_razorpay_content = '''                  <p style="margin-top:0; font-size:16px;">Hi <b>{{USERNAME}}</b>,</p>
...
templates = {
    "OTP_MAIL": build_template("Account Verification", "🔐", otp_content),
    "PASSWORD_RESET_MAIL": build_template("Password Reset Request", "🔑", reset_content),
    "ORDER_CONFIRMED_MAIL": build_template("Order Confirmed", "✨", order_conf_content),
    "PAYMENT_FAILED_MAIL": build_template("Payment Failed", "💳", pay_fail_content),
    "WELCOME_MAIL": build_template("Welcome Aboard", "👋", welcome_content),
    "SELLER_ORDER_NOTIFICATION_MAIL": build_template("New Order Received", "🎉", seller_order_content),
    "WELCOME_SELLER_MAIL": build_template("Merchant Hub Access", "🏪", welcome_seller_content),
    "ORDER_SHIPPED_MAIL": build_template("Order Shipped", "🚚", order_shipped_content),
    "ORDER_DELIVERED_MAIL": build_template("Order Delivered", "📦", order_delivered_content),
    "MISSING_RAZORPAY_MAIL": build_template("Action Required: Payout", "⚠️", missing_razorpay_content)
}

java_content = "package com.raunak.library.email_service.constants;\n\npublic class MailTemplate {\n"
for name, html in templates.items():
    java_content += f'    public static final String {name} = """\n{html}\n""";\n\n'
java_content += "}\n"

with open(r'c:\\Users\\rauna\\Desktop\\Books-Management-System\\email-service\\email-service\\src\\main\\java\\com\\raunak\\library\\email_service\\constants\\MailTemplate.java', 'w', encoding='utf-8') as f:
    f.write(java_content)

print("Generated MailTemplate.java")
